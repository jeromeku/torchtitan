...
def test_comm_mode_with_dtensor(mesh, device_type="cuda", ):
    ...
    with comm_mode:
        out: DTensor = f(x_dtensor, y_dtensor)
    print(f"{type(out)} {out.shape} {out._local_tensor.shape=} {out.placements}")

    comm_counts = comm_mode.get_comm_counts()
    from pprint import pprint

    comms_table = comm_mode.generate_comm_debug_tracing_table(noise_level=3)
    parameter_info = comm_mode.get_parameter_info()
    shard_info = comm_mode.get_sharding_info()

    # pprint(dict(comm_counts))
    # pprint(parameter_info)
    # pprint(shard_info)
    # print(comms_table)
    # self.assertEqual(comm_mode.get_total_counts(), 1)
    # self.assertEqual(comm_counts[c10d_functional.all_reduce], 0)
...
if __name__ == "__main__":
    ...
    with dist_context(world_size=world_size):
        mesh = DeviceMesh(device_type, list(range(world_size)))
        test_comm_mode_with_dtensor(mesh=mesh)